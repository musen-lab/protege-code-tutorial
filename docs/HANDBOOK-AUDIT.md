...
| OSGi and JPF history | Intentionally secondary | The course explains the current Felix plus Equinox mechanism. The older Protégé and JPF chronology is useful background but not required on the primary causal path, so it remains handbook reference material rather than a new lesson. |
